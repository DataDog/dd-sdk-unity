...
// System.Diagnostics.EventLog reference when building a Player. Restricting these DLLs to
// Editor-only via the PluginImporter API (never hand-edited YAML) removes them from every
// Player build target while leaving the EditMode test assemblies' explicit
// precompiledReferences resolution (NSubstitute.dll) untouched. This runs automatically before
// every Player build (via IPreprocessBuildWithReport), so it stays correct even after
// NuGetForUnity re-adds or upgrades any of the listed packages.
public class DatadogScaffoldPluginSettings : IPreprocessBuildWithReport
{
    ...
    {
        RestrictTestPluginsToEditor();
    }

    private static void RestrictTestPluginsToEditor()
    {
        var updated = 0;

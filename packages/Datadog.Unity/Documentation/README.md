...
In this example, the log message that appears in Datadog will show the full details of the exception, including the stack trace. If the exception occurred in native code generated by IL2CPP, Datadog will show the full native stack trace.
...
You may also construct an `ErrorInfo` manually in order to provide the Datadog SDK with arbitrary error information. However, you should prefer passing `Exception` values where possible, since native IL2CPP stack traces can only be recovered from a C# `Exception`.
